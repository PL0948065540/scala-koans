...
class AboutLists extends KoanSuite {
  ...
  koan("Lists can be 'reduced' with a mathematical operation") {
    ...
    a.reduceLeft(_ + _) should equal(1 + 3 + 5 + 7)
    a.reduceLeft(_ * _) should equal(1 * 3 * 5 * 7)
  }
  ...
  koan("Foldleft is like reduce, but with an explicit starting value") {
    ...
    a.foldLeft(0)(_ + _) should equal(1 + 3 + 5 + 7)
    a.foldLeft(10)(_ + _) should equal(10 + 1 + 3 + 5 + 7)
    a.foldLeft(1)(_ * _) should equal(1 * 1 * 3 * 5 * 7)
